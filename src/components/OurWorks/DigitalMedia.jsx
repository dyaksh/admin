...
import AddDegitalMedia from "./AddWorks/AddDegitalMedia";
import ViewDigitalMedia from "./ViewWorks/ViewDigitalMedia";

const DigitalMedia = () => {

    const [searchQuery, setSearchQuery] = useState('');
    const [filteredItems, setFilteredItems] = useState([])
    const [addDigitalMedia, setDigitalMedia] = useState(false);
    const [viewDigitalMedia, setViewDigitalMedia] = useState(false)
    const [selectedDigitalMedia, setSelectedDigitalMedia] = useState("")
    const [items, setItems] = useState([
        {
            id: 1,
            title: "Project Alpha",
            description: "A gana of Project Alpha.",
    ...
    const handleAddDigitalMedia = value => {
        setDigitalMedia(value);
    }
    const handleView = id =>{
        const temp = items.filter(item => item.id === id)
        setSelectedDigitalMedia(temp[0]);
        setViewDigitalMedia(true);
    }
    const handleViewDigitalMedia = value => {
        setViewDigitalMedia(value);
    }
    ...
    return (
        <>
            <div className="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5">
                <div className="mb-1 w-full">
                    <div className="mb-4">
                        <nav className="flex mb-5" aria-label="Breadcrumb">
                            <ol className="inline-flex items-center space-x-1 md:space-x-2">
                                ...
                                <li>
                                    <div className="flex items-center">
                                        <svg className="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd"></path></svg>
                                        <Link to="#" className="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Digital Media</Link>
                                    </div>
                                </li>
                                <li>
                                    <div className="flex items-center">
                        ...
                        <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Digital Media</h1>
                    </div>
                    <div className="sm:flex">
                        <div className="hidden sm:flex items-center sm:divide-x sm:divide-gray-100 mb-3 sm:mb-0">
                            <form className="lg:pr-3" action="#" method="GET">
                                ...
                                <div className="mt-1 relative lg:w-64 xl:w-96">
                                    <input type="text" name="email" id="users-search" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Search for Digital Media" value={searchQuery}
                        ...
                        <div className="flex items-center space-x-2 sm:space-x-3 ml-auto">
                            <button type="button" onClick={() => setDigitalMedia(true)} data-modal-toggle="add-user-modal" className="w-1/2 text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium inline-flex items-center justify-center rounded-lg text-sm px-3 py-2 text-center sm:w-auto">
            ...
            <AddDegitalMedia addDigitalMedia={addDigitalMedia} handleAddDigitalMedia={handleAddDigitalMedia} />
            <ViewDigitalMedia viewDigitalMedia={viewDigitalMedia} handleViewDigitalMedia={handleViewDigitalMedia}
                title={selectedDigitalMedia.title}
                image={selectedDigitalMedia.image}
                description={selectedDigitalMedia.description}
                url = {selectedDigitalMedia.url}
            />
        </>
    )
}
