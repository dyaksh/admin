...
import AddTvComDocs from "./AddWorks/AddTvComDocs";
import ViewTvComDocs from "./ViewWorks/ViewTvComDocs";

const TvComDocs = () => {
    const [searchQuery, setSearchQuery] = useState('');
    const [filteredItems, setFilteredItems] = useState([]);
    const [addTvComDocs, setAddTvComDocs] = useState(false);
    const [viewTvComDocs, setViewTvComDocs] = useState(false)
    const [selectedTvComDocs, setSelectedTvComDocs] = useState("")
    ...
    const handleAddTvComDocs = value => {
        setAddTvComDocs(value);
    }

    const handleView = id =>{
        const temp = items.filter(item => item.id === id)
        setSelectedTvComDocs(temp[0]);
        setViewTvComDocs(true);
    }
    const handleViewTvComDocs = value => {
        setViewTvComDocs(value);
    }
    ...
    return (
        <>
            <div className="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5">
                <div className="mb-1 w-full">
                    <div className="mb-4">
                        <nav className="flex mb-5" aria-label="Breadcrumb">
                            <ol className="inline-flex items-center space-x-1 md:space-x-2">
                                ...
                                <li>
                                    <div className="flex items-center">
                                        <svg className="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd"></path></svg>
                                        <Link to="#" className="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Tv Com Docs</Link>
                                    </div>
                                </li>
                                <li>
                                    <div className="flex items-center">
                        ...
                        <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Tv COM docs</h1>
                    </div>
                    <div className="sm:flex">
                            ...
                            <form className="lg:pr-3" action="#" method="GET">
                                ...
                                <div className="mt-1 relative lg:w-64 xl:w-96">
                                    <input type="text" value={searchQuery}
                                        onChange={(e) => setSearchQuery(e.target.value)} name="email" id="users-search" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Search for Tv COM docs" />
                                </div>
                            </form>
                        ...
                        <div className="flex items-center space-x-2 sm:space-x-3 ml-auto">
                            <button type="button" onClick={() => setAddTvComDocs(true)} data-modal-toggle="add-user-modal" className="w-1/2 text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium inline-flex items-center justify-center rounded-lg text-sm px-3 py-2 text-center sm:w-auto">
            ...
            <AddTvComDocs addTvComDocs={addTvComDocs} handleAddTvComDocs={handleAddTvComDocs} />
            <ViewTvComDocs viewTvComDocs={viewTvComDocs} handleViewTvComDocs={handleViewTvComDocs}
                title={selectedTvComDocs.title}
                image={selectedTvComDocs.image}
                description={selectedTvComDocs.description}
                url = {selectedTvComDocs.url}
            />
        </>
    )
}

export default TvComDocs