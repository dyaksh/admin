...
const ViewTvComDocs = ({ viewTvComDocs, handleViewTvComDocs, title, description, image, url }) => {
    const setViewTvComDocs = () => {
        handleViewTvComDocs(false);
    };

    return (
        <>
            <div className={`${viewTvComDocs ? 'flex' : 'hidden'} fixed inset-0 z-50 justify-center items-center bg-black bg-opacity-50`} id="exhibition-modal" aria-hidden={!viewTvComDocs && "true"} aria-modal={viewTvComDocs && "true"} role={viewTvComDocs && 'dialog'}>
                <div className="relative w-screen max-w-md px-4 h-full md:h-auto">
                    ...
                    <div className="bg-white rounded-lg shadow-lg relative">
                        ...
                        <div className="flex items-start justify-between p-5 border-b rounded-t">
                            ...
                            <button type="button" onClick={setViewTvComDocs} className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="exhibition-modal">
                        ...
                        <div className="p-6 border-t border-gray-200 rounded-b text-center">
                            <button className="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button" onClick={setViewTvComDocs}>Close</button>
                        </div>
                    </div>
                </div>
            </div>
